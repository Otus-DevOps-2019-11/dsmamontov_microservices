...
# kubernetes-5
```
настроен мониторинг k8s-кластера и мониторинг приложения с помощью прометеуса и графаны
настроены старые дашборды с динамическими переменными (папка monitoring/grafana/dashboard-templating)
настроены правила доступности сервисов из предыдущих дз для алерт-менеджера (up)

прометеус-оператор можно заинсталлировать, например, так:
kubectl create namespace monitoring
helm install --name monitoring-operators --namespace monitoring --set rbac.create=true stable/prometheus-operator -f prometheus-operator-values.yaml

так же установлен efk-stack
